...
def parse_log_file(filepath):
    ...
    current_data_line = ""
    partition_times = []
    first_partition_block_processed = False # Nouveau drapeau
    nb_partitions = 8
    compteur = 0
    with open(filepath, 'r') as f:
        for line in f:
            ...
            # Capture les temps de remaillage des partitions seulement pour le premier bloc
            if stripped_line.startswith("partition ") and not first_partition_block_processed:
                try:
                    ...
                    compteur += 1
                    if compteur == nb_partitions:
                        first_partition_block_processed = True; 
                except (IndexError, ValueError):
                    continue

            # Détecte le début d'un nouveau bloc DATA
            elif stripped_line.startswith("DATA,"):
                # Si un bloc DATA précédent existe, le traite et l'ajoute
...
for name, group in plot_grouped:
    ...
    if not nocost_data.empty:
        plt.plot(nocost_data['num_elements'], nocost_data['remeshing_partition_time_mean'],
                 '--', color='orange', label='Partition Time (No Load Balancing)', alpha=0.6)
        ...
        plt.plot(nocost_data['num_elements'], nocost_data['remeshing_ifc_time_mean'],
                 '--', color='red', label='Interface Time (No Load Balancing)', alpha=0.6)
        plt.fill_between(nocost_data['num_elements'], 
    ...
    if not totocost_data.empty:
        plt.plot(totocost_data['num_elements'], totocost_data['remeshing_partition_time_mean'],
                 '--', color='cyan', label='Partition Time (With Load Balancing)', alpha=0.6)
        ...
        plt.plot(totocost_data['num_elements'], totocost_data['remeshing_ifc_time_mean'],
                 '--', color='blue', label='Interface Time (With Load Balancing)', alpha=0.6)
        plt.fill_between(totocost_data['num_elements'], 
    ...
    if not nocost_data.empty:
        plt.errorbar(nocost_data['num_elements'], nocost_data['total_remeshing_time_mean'],
                     ...
                     label='Total Remeshing Time (No Load Balancing)', color='orange')
    if not totocost_data.empty:
        plt.errorbar(totocost_data['num_elements'], totocost_data['total_remeshing_time_mean'],
                     ...
                     label='Total Remeshing Time (With Load Balancing)',color='blue')

    plt.title(f'Total Remeshing Time by Element Count ({metric_type}, Part: {partitioner}, Op: {metric_op})')
...
for name, group in plot_grouped_disparity:
    ...
    for i, num_elements in enumerate(unique_num_elements):
        ...
        if not nocost_data.empty:
            ...
            labels.append(f'No Load Balancing \n({num_elements})')
        ...
        if not totocost_data.empty:
            ...
            labels.append(f' Load Balancing \n({num_elements})')
    ...
    legend_elements = [
        plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='orange', markersize=10, label='No Load Balancing'),
        plt.Line2D([0], [0], marker='x', color='w', markerfacecolor='cyan', markeredgecolor='cyan', markersize=10, label='Load Balancing')
    ]
    
    plt.title(f'Partition Remeshing Time Disparity ({metric_type}, Part: {partitioner}, Op: {metric_op})')
    plt.xlabel('Element Count')
    plt.ylabel('Partition Remeshing Time (seconds)')
    plt.grid(True)
